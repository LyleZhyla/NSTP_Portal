...
$studentSql = "
    SELECT
        ...
        COALESCE(NULLIF(creator.program, ''), 'N/A') AS component
    FROM tbl_student s
...
if ($selectedFacilitatorId && $selectedSection !== '') {
    $studentSql .= " WHERE s.created_by = :selected_facilitator_id AND s.course_section = :selected_section";
    $studentParams[':selected_facilitator_id'] = $selectedFacilitatorId;
    $studentParams[':selected_section'] = $selectedSection;
} elseif ($role === 'coordinator') {
    if ($program === 'ROTC') {
        $studentSql .= ' WHERE ' . rotcStudentSqlCondition('s');
    } else {
        $studentSql .= "
            WHERE (creator.role = 'facilitator' AND creator.program = :program)
