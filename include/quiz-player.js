(function(){
    ...
    function play(ctx,loaded,preview=false){
        const app=ctx.app,d=loaded.definition;let answers=loaded.response?.answers||{},editing=loaded.response?.state==='submitted',done=false,changed=false,saveTimer=null,queue=Promise.resolve(),uploads=0,version=0,heartbeat=null;
        app.style.setProperty('--quiz-accent',d.accent);
        function header(){return `<div class="card quiz-top"><div class="card-body"><h1 class="h3">${e(d.title)}</h1><p class="quiz-help">${e(d.description)}</p>${preview?'<div class="alert alert-info mb-0">Preview only. Answers and scores are not saved.</div>':`<p class="small quiz-muted mb-0">Your name is recorded from your student account. ${d.closes_at?'Closes '+e(d.closes_at.replace('T',' '))+' (Manila time).':''}</p>`}</div></div>`;}
        function stop(){clearInterval(heartbeat);clearTimeout(saveTimer);}
        function render(){
            ...
            form.addEventListener('submit',async event=>{event.preventDefault();if(uploads||next.disabled)return;collect();if(!validate(form.querySelectorAll('[data-section]')[current],d,answers))return;const target=nextIndex();if(target<blocks.length){history.push(current);current=target;show();form.querySelectorAll('[data-section]')[current].scrollIntoView({behavior:'smooth',block:'start'});return;}if(preview){app.innerHTML=header()+`<div class="card p-4"><h2 class="h4">Preview complete</h2><p>${e(d.confirmation)}</p><p class="quiz-muted">No response was saved.</p><button class="btn btn-outline-success" id="preview-restart">Try again</button></div>`;app.querySelector('#preview-restart').onclick=()=>{answers={};render();};return;}
                ...
                try{await queue;const result=await ctx.api('submit',{id:ctx.id,answers:collect()});done=true;changed=false;stop();location.href=`quiz.php?id=${ctx.id}&mode=result&response_id=${result.response_id}`;}catch(error){ctx.message(error.message);draftStatus.textContent='Not submitted. Your answers remain on this page.';next.disabled=false;back.disabled=false;}
            });
            show();
            if(!preview){heartbeat=setInterval(()=>document.dispatchEvent(new Event('nstp:upload-activity')),30000);window.addEventListener('beforeunload',event=>{if(changed&&!done){event.preventDefault();event.returnValue='';}});}
        }
        ...
        if(editing){app.innerHTML=header()+`<div class="card p-4"><h2 class="h4">Response submitted</h2><div class="quiz-actions"><a class="btn btn-success" href="quiz.php?id=${ctx.id}&mode=result&response_id=${loaded.response.response_id}">View my response</a>${d.allow_edit?'<button class="btn btn-outline-success" id="edit-my-response">Edit response</button>':''}</div></div>`;app.querySelector('#edit-my-response')?.addEventListener('click',render);return stop;}
        ...
        app.querySelector('#quiz-start').onclick=async event=>{event.target.disabled=true;try{const response=await ctx.api('start',{id:ctx.id});answers=response.answers||{};render();}catch(error){ctx.message(error.message);event.target.disabled=false;}};
        return stop;
    }
