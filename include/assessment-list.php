...
try {
    ...
    $stmt=$conn->prepare('SELECT m.quiz_id,m.title,m.description,m.status,m.uploaded_by,m.audience_components,m.audience_rotc_levels,m.definition_json,r.response_id,r.state AS response_state'.$quizJoin." ORDER BY m.quiz_id DESC LIMIT 20 OFFSET {$quizOffset}");$stmt->execute($quizParams);$quizzes=$stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<article class="border rounded p-3 mb-3">
    <h3 class="h5"><?= quizEscape($quiz['title']) ?> <span class="badge badge-<?= $quiz['status']==='published'?'success':'secondary' ?>"><?= quizEscape(ucfirst($quiz['status'])) ?></span><?php if($externalQuiz): ?> <span class="badge badge-info">Google Form</span><?php endif; ?></h3>
    ...
    <?php if ($manage): ?>
        ...
        <a class="btn btn-outline-secondary btn-sm" href="quiz.php?id=<?= (int)$quiz['quiz_id'] ?>&amp;mode=preview"><?= $externalQuiz?'Preview Google Form':'Preview' ?></a>
        <?php if(!$externalQuiz): ?><a class="btn btn-outline-success btn-sm" href="quiz.php?id=<?= (int)$quiz['quiz_id'] ?>&amp;mode=responses">Responses</a><?php endif; ?>
    ...
    <?php else: ?>
        <a class="btn btn-success btn-sm" href="quiz.php?id=<?= (int)$quiz['quiz_id'] ?>"><?= $externalQuiz?'Open Google Form':($materialActor['role']==='student'?($quiz['response_state']==='draft'?'Continue Quiz':'Open Quiz'):'Preview Quiz') ?></a>
    <?php endif; ?>
</article>
<?php endforeach; ?>
